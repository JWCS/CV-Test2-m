        ...
        for y = 1:Y
            if seen( x, y ) ~= 0
                ...
                while onLine
                    ...
                    [xN, yN, allZ] = nextHigh( C, p2, seen.*ReducedIm);
                    p2 = [xN, yN];
...
function [ xN, yN, allZ ] = nextHigh( C, p2, Im )
%nextHigh Returns xN, yN point of the highest ~= 0 point in the mat
    x1 = C(1); y1 = C(2); x2 = C(3); y2 = C(4); struct = C(5); dir = C(6);
    Im1Col = Im(x1:x2,y1:y2);
    [ ~, I ] = max(Im1Col(:));
    [ px, py ] = ind2sub( size( Im1Col ), I );
    [xN, yN] = searchMat2Im(p2, px, py, struct, dir);
...
function [pX, pY] = searchMat2Im( p0, px, py, struct, dir)
